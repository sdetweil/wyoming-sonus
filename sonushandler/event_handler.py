...
class SonusEventHandler(AsyncEventHandler):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.speak: bool = self.config_info['voice_output']    
        _LOGGER.debug("done w init")

    async def runit(self) -> None:
        self.setup()
        if self.cli_args.socket_address is not None:
             self.socketClient =socketio.AsyncClient()
             try: 
                await self.socketClient.connect(self.cli_args.socket_address)        
             except:
                _LOGGER.debug("socket connect failed")
                self.socketClient = None
    ...
    async def started(self) -> None:
        ...
        while  True:
            await asyncio.sleep(24*60*60*1000)  # one day
    ...
    async def handle_event(self, event: Event) -> bool:
        ...
        #            
            ...
            print("===>hotword<===")
            await self.notifyManager("hotword")
            sys.stdout.flush()            
            await self.sendEvent(Transcribe().event(), self._asr_queue)   
            try: 
                async with asyncio.timeout(self.config_info["vad"]["wake_word_timeout"]):                
                  self.reco_timeout_task= asyncio.create_task(self.sleeproutine(self.config_info["vad"]["wake_word_timeout"]+1)) # force longer than 
                  await self.reco_timeout_task
                                       
            except TimeoutError:                
                    self.state=State.LISTENING
                    self.reco_timeout_task=None
                    _LOGGER.debug("transcript timeout")
                    await self.sendEvent(AudioStop().event(), self._asr_queue)
        #
        ...
        #        
            ...
            if self.config_info["voice_output"] is not None and  not 0:
              self.state = State.WAITING
            else:
              self.state = State.IDLE  
            ...
            await self.notifyManager(self.transcript)
            sys.stdout.flush()
    ...
    '''
    async def notifyManager(self,text:str)->None:
        if self.socketClient is not None:
           await self.socketClient.emit(text)
